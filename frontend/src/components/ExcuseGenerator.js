// ExcuseGenerator component is responsible for fetching excuses from the backend and displaying them to the user
...
const ExcuseGenerator = () => {
  ...
  const [usedExcuses, setUsedExcuses] = useState([]); // Used excuses
  const [empty] = useState("Need an excuse ? We've got you 🫵"); // Empty state, before first click
  const [firstClick, setFirstClick] = useState(false); // Track first click
  const [loading, setLoading] = useState(false); // Track loading state
  const [showButtonRandom, setShowButtonRandom] = useState(false); // Track if random button should be shown
  const [showButtonModal, setShowButtonModal] = useState(false); // Track if modal should be shown

  // Fetch excuses from the API
  const updateExcuses = () => {
    axios
  ...
  // Trigger button appearance after delay
  useEffect(() => {
  ...
  const generateExcuse = () => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
    }, Math.floor(Math.random() * 4000) + 1000); // LOADER / random delay between 1s and 5s
  };

  return (
    <div className={`excuse-generator ${firstClick ? 'first-click' : ''}`}>
      <div className={`title-gen ${firstClick ? 'small-title' : ''}`}>
        <h1>{firstClick ? '- excuses de dev -' : 'Les excuses de dev'}</h1>
      </div>
